...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    output = tf.layers.conv2d_transpose(add2, 2 * num_classes, 16, strides = (8,8), padding = 'same',
                                    kernel_regularizer = tf.contrib.layers.l2_regularizer(1e-3),
                                    kernel_initializer = tf.random_normal_initializer(stddev = 0.01))
...
def optimize(nn_last_layer, correct_label, learning_rate, num_classes):
    ...
    logits = tf.reshape(nn_last_layer, (-1, 2 * num_classes), name = 'logits')
    correct_label = tf.reshape(correct_label, (-1, 2 * num_classes))
...
def gen_batch_function(data_folder, image_shape):
    ...
    def get_batches_fn(batch_size):
        ...
        label_paths_road = glob(os.path.join(data_folder, 'labeled', 'road', '*.png'))
...
#                 image = scipy.misc.imresize(scipy.misc.imread(image_file), image_shape)
                ...
                gt_image_file_car = label_paths_car[i]
                
#                 gt_image_road = scipy.misc.imresize(scipy.misc.imread(gt_image_file_road), image_shape) 
                gt_image_car = scipy.misc.imread(gt_image_file_car) 

                gt_bg_car = np.all(gt_image_car == background_color, axis=2)
                gt_bg_car = gt_bg_car.reshape(*gt_bg_car.shape, 1)
                gt_image_car = np.concatenate((gt_bg_car, np.invert(gt_bg_car)), axis=2)
...
#                 gt_image_road = scipy.misc.imresize(scipy.misc.imread(gt_image_file_road), image_shape) 
                ...
                gt_image = np.concatenate((gt_image_car, gt_image_road), axis=2)
...
def run():
    ...
    correct_label = tf.placeholder(tf.int32, shape = [None, None, None, 2 * num_classes])
    ...
    with tf.Session() as sess:
        ...
        # Save GraphDef
        tf.train.write_graph(sess.graph_def,'.','lyft_final_model.pb')
        # Save checkpoint
        saver.save(sess=sess, save_path="./lyft_final_model")
